...
# Rutas para entorno local / GitHub Actions
...
# --- REGLAS OFICIALES EXTRAÍDAS DEL PÓSTER ---
ROUND_RULES = {
    "R1": {"ALLOWED_CLASS": "CUP"},       # Ferrari Challenge Cars
    "R2": {"ALLOWED_CLASS": "TCX"},       # TCX
    "R3": {"ALLOWED_CLASS": "CUP"},       # Cup Cars
    "R4": {"ALLOWED_CLASS": "GT4"},       # GT4s
    "R5": {"ALLOWED_BRANDS": ["HONDA", "NISSAN"]}, # Honda & Nissan only
    "R6": {"ALLOWED_CLASS": "GT4"},       # GT4s
    "R7": {"ALLOWED_CLASS": "GT2"},       # GT2s
    "R8": {"MIXED": True}                 # Mixed Class / Unique to bracket
}
...
def get_car_class(car_model, car_group=""):
    ...
    if "GT2" in grp: return "GT2"
    if "CUP" in grp or "GTC" in grp or "CHL" in grp or "ST" in grp: return "CUP"
    return "GT3"

def is_car_brand_honda_or_nissan(car_model):
    """Identifica si el modelo corresponde a Honda o Nissan en ACC"""
    try:
        model = int(car_model)
    except:
        return False
...
def check_legality(car_model, car_class, round_rule):
    """Comprueba si el piloto cumple estrictamente con el reglamento de la ronda"""
    if not round_rule or round_rule.get("MIXED"):
        return True
        
    if "ALLOWED_CLASS" in round_rule:
        return car_class == round_rule["ALLOWED_CLASS"]
        
    if "ALLOWED_BRANDS" in round_rule:
        brands = round_rule["ALLOWED_BRANDS"]
        if "HONDA" in brands or "NISSAN" in brands:
            return is_car_brand_honda_or_nissan(car_model)
...
def load_and_process():
    ...
    for r_key in round_keys:
        ...
        round_races_data = []
        round_rule = ROUND_RULES.get(r_key, {})
        ...
        for file_index, race_data in enumerate(race_sessions):
            ...
            if track_name not in hall_of_fame:
                ...
                for cls_name in ["GT3", "GT4", "TCX", "CUP", "GT2"]:
            ...
            if q_data:
                ...
                for line in q_leaderboard:
                    ...
                    c_class = get_car_class(line['car'].get('carModel', 0), line['car'].get('carGroup', ''))
                ...
                q_driver_cars = {line['car']['carId']: f"{line['currentDriver']['firstName']} {line['currentDriver']['lastName']}".strip() + "::" + get_car_class(line['car'].get('carModel', 0), line['car'].get('carGroup', '')) for line in sorted_q_lines}
            ...
            for lap in race_data.get('laps', []):
                ...
                if ltime < 2000000000:
                    t_107 = threshold_107.get(c_class, 0)
                    is_incident = t_107 > 0 and ltime > t_107
            ...
            for line in race_leaderboard:
                car_model = line['car'].get('carModel', 0)
                c_class = get_car_class(car_model, line['car'].get('carGroup', ''))
                ...
                gets_points = is_legal and (laps >= min_laps_classified.get(c_class, 0))
                race_gap_str = "-"
                ...
                temp_drivers.append({
                    ...
                    "total_time_ms": total_time if gets_points else 9999999999, 
            ...
            for cls in ["GT3", "GT4", "TCX", "CUP", "GT2"]:
        ...
        for r_data in round_races_data:
            for d in r_data['temp_drivers']:
                ...
                if d['gets_points']:
                    ...
                    round_standings[pid]['pos_sum'] += 99 
        ...
        for i, r_data in enumerate(round_races_data):
            ...
            for d in r_data['temp_drivers']:
                ...
                d_export = d.copy()
                for key in ['pid', 'has_stats', 'gets_points', 'real_pos_num', 'has_valid_pace_gap', 'total_time_ms', 'is_legal']:
    ...
    print(f"✅ Dashboard FF actualizado con reglas de temporada: {OUTPUT_FILE}")
