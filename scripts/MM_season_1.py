...
OUTPUT_FILE = "public/data/monday_marathon/season_1.json"
...
def extract_date_from_filename(filename):
    ...
    # 1. Buscar formato ACC automático: 260831_210107 (AAMMDD_HHMMSS)
    match_acc = re.search(r'(\d{2})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})', base_name)
    if match_acc:
        yy, mm, dd, hh, mins, ss = match_acc.groups()
        return f"20{yy}-{mm}-{dd} {hh}:{mins}:{ss}"
        
    # 2. Buscar formato manual tuyo: 6-4-26 (D-M-YY)
    match_manual = re.search(r'(\d{1,2})-(\d{1,2})-(\d{2})', base_name)
    if match_manual:
        d, m, yy = match_manual.groups()
        return f"20{yy}-{int(m):02d}-{int(d):02d} 00:00:00"
        
    # Si no encuentra nada, devolvemos una fecha por defecto para que no explote
...
def load_and_process():
    ...
    for f, data in archivos_parseados:
        ...
        if session_type == 'Q':
            ...
            data['timestamp_carrera'] = extract_date_from_filename(f)
    ...
    for file_index, race_data in enumerate(race_sessions):
        ...
        session_list.append({
            ...
            "timestamp": race_data.get('timestamp_carrera', "1970-01-01 00:00:00"),
